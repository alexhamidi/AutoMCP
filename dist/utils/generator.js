...
async function generateMcp(tools, name) {
...
function generateToolsSetup(tools, name) {
...
function getFunctionStr(tool, name) {
...
function generateTools(tools, name) {
    const toolCalls = tools.map(tool => {
        const schema = getSchemaStr(tool);
        return `  server.tool(
    "${tool.name}",
